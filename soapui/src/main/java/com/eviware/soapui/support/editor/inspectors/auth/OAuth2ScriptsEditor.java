...
/**
 * Component that allows a user to edit the JavaScript snippets associated with
 * an OAuth 2 flow.
 */
public class OAuth2ScriptsEditor extends JPanel {
	...
	private JXToolBar buildToolbar(final OAuth2Profile profile) {
		...
		toolbar.addGlue();
		toolbar.add(
				UISupport.createToolbarButton(new ShowOnlineHelpAction(HelpUrls.OAUTH_AUTOMATED_TOKEN_PROFILE_EDITOR)));
		return toolbar;
	}
	...
	private JPanel makeScriptsPanel(final OAuth2Profile profile) {
		...
		for (int index = 0; index < numberOfFields; index++) {
			String scriptName = (index < DEFAULT_SCRIPT_NAMES.length ? DEFAULT_SCRIPT_NAMES[index]
					: "Page " + (index + 1));
		}
	...
	class InputPanel extends JPanel {
		...
		private Color aDarkerShadeThan(Color color) {
			return new Color((int) (color.getRed() * .9), (int) (color.getBlue() * .9), (int) (color.getGreen() * .9));
		}
	...
	private class JavaScriptErrorReporter extends BrowserListenerAdapter {
		...
		public void javaScriptExecuted(final String script, final String errorLocation, final Exception error) {
			...
			if (error != null) {
				hasErrors = true;
				// invokeLater() is necessary, because the call comes from the JavaFX invoker
				// thread
					...
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							UISupport.showInfoMessage(
									"The scripts could only be partially validated, because all scripts "
